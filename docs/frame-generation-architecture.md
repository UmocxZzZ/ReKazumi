...
- mpv VO 按源 PTS 调度原始、`1/3`、`2/3` 三相；错过截止时间只丢生成相，
  不追赶、不改变音频或播放速度。
- 帧生成直接位于 `gpu-next`，复用同一个 libplacebo `pl_gpu`。源帧颜色转换使用
  独立 renderer 以隔离 Anime4K/HDR 主渲染缓存，但不创建第二个 Vulkan device
  或 queue。
- 全局运动使用 33×33 候选并行计算（覆盖 ±64 像素、步长 4）和 1×1 归约；
  每个 16×16 块再以 17×17 候选图并行细化（±16 像素、步长 2），之后逐块归约。
- 成本图与置信度使用 RG16F，源帧、运动向量和生成输出使用 RGBA16F；1080p
  局部候选临时纹理约 9.4 MB。生成帧永远不作为下一帧对输入。
- 双向 warp 使用前后向一致性权重；近重复块和高代价/切镜块直接保持前一原始帧。
  日志分别报告全局搜索、局部搜索和 warp 的 GPU 时间。
- 任意资源、shader 或 dispatch 失败都会将倍率立即熔断回 1×。当前应用仍固定使用
  已验证的 fail-closed 运行库，设置层不得把该原型标记为可用。

原生 CI 通过只证明补丁可应用、arm64 可编译、标记与禁用项正确；它不证明运行时
shader、画质、帧时或显示栈安全。下一道门仍是与最终 Android Vulkan 环境一致的
离屏输出和计时验证，之后才允许短时呈现。
